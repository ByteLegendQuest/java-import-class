...
`git clone https://github.com/ByteLegendQuest/java-import-class`

A project directory named `java-import-class` will be created, which can be moved wherever you like.

Open `IDEA`, `File` - `Open`, select and open the `pom.xml` in `java-import-class` directory which is created just now.
