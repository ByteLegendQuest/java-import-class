...
-   Complete `import` s en la clase `Home` para que el código pueda compilarse.
...
-   أكمل عمليات `import` في فئة `Home` حتى يمكن ترجمة التعليمات البرمجية.
...
-   Complete `import` s na classe `Home` para que o código possa compilar.
...
-   Selesaikan `import` s di kelas `Home` agar kode dapat dikompilasi.
...
-   Terminez `import` dans la classe `Home` afin que le code puisse être compilé.
...
-   コードをコンパイルできるように、 `Home`クラスの`import`を完了します。
...
-   Завершите `import` в классе `Home` , чтобы код можно было скомпилировать.
...
-   Schließen Sie `import` s in der `Home` -Klasse ab, damit der Code kompiliert werden kann.
...
-   코드가 컴파일될 수 있도록 `Home` 클래스에서 `import` s를 완료하십시오.
...
-   Completa i messaggi di `import` nella classe `Home` in modo che il codice possa essere compilato.
